"""
Simple test for the parameter bridge functions.
"""
...
def test_tracking_params_bridge():
    ...
    # Convert to a dictionary for C struct
    c_struct_dict = params.to_c_struct()
    print("\nC struct dictionary:")
    for key, value in c_struct_dict.items():
        print(f"{key}: {value}")

    # Create a Cython TrackingParams object
    cython_params = CythonTrackingParams(
        ...
        dacc=params.dacc,
        add=1 if params.flagNewParticles else 0
    )

...
on the test_cavity directory and verifies all steps.
"""
import pytest
import os
import sys
...
# Import the patched version of pyptv_batch
from tests.patched_pyptv_batch import main
...
def test_pyptv_batch_full_sequence(test_data_dir, setup_and_cleanup_res_dir):
    """
    ...
    try:
        ...
        # Check if we have ptv_is.XXXXX files which indicate tracking was done
        tracking_files = list(res_dir.glob("ptv_is.*"))
        if tracking_files:
            print(f"\nTracking completed successfully! Found {len(tracking_files)} tracking files.")

            # Check if we have rt_is.XXXXX files which indicate correspondence was done
            ...
            # Check if we have added.XXXXX files
            ...
            assert len(tracking_files) == (end_frame - start_frame + 1), f"Expected {end_frame - start_frame + 1} tracking files, found {len(tracking_files)}"
            assert len(corresp_files) == (end_frame - start_frame + 1), f"Expected {end_frame - start_frame + 1} correspondence files, found {len(corresp_files)}"
            assert len(added_files) == (end_frame - start_frame + 1), f"Expected {end_frame - start_frame + 1} added particles files, found {len(added_files)}"
        else:
...
if __name__ == "__main__":
    ...
    print("\n=== Running Full PyPTV Batch Sequence Test ===\n")
    ...
    try:
        ...
        # Run the test
        test_pyptv_batch_full_sequence(test_data_dir, None)  # None as placeholder for the fixture
        print("\n✅ Full PyPTV batch sequence test passed successfully!")
        sys.exit(0)
    except Exception as e:
        print(f"\n❌ Full PyPTV batch sequence test failed: {str(e)}")
        sys.exit(1)
    finally:
        # Clean up: remove the res directory created during the test
        if res_dir.exists():

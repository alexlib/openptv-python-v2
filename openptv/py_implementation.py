...
try:
    from openptv.pyoptv import (
        calibration as py_cal,
        tracking as py_track,
        correspondences as py_corresp,
        image_processing as py_imgproc
    )
except ImportError:
    raise ImportError("Python implementation modules not available")
...
class PyPTVImplementation(PTV_Interface):
    ...
    def read_calibration(self, ori_file, addpar_file=None):
        ...
        return py_cal.read_calibration(ori_file, addpar_file)
    
    def write_calibration(self, cal_obj, ori_file, add_file=None):
        return py_cal.write_calibration(cal_obj, ori_file, add_file)
    
    def calibration_ori(self, cal_obj, known_points, image_points, cpar):
        return py_cal.calibration_ori(cal_obj, known_points, image_points, cpar)
    
    def full_calibration(self, cal_obj, known_points, image_points, cpar, flags):
        return py_cal.full_calibration(cal_obj, known_points, image_points, cpar, flags)
    
    def external_calibration(self, cal_obj, known_points, image_points, cpar, flags):
        return py_cal.external_calibration(cal_obj, known_points, image_points, cpar, flags)
    
    def point_position(self, point_2d, cal_obj, flags):
        return py_cal.point_position(point_2d, cal_obj, flags)
    
    def image_coordinates(self, point_3d, cal_obj, flags):
        return py_cal.image_coordinates(point_3d, cal_obj, flags)
    
    def distort_point_positions(self, pos, ap, flags):
        return py_cal.distort_point_positions(pos, ap, flags)
    
    def remove_distortion(self, pos, cal_obj, flags):
        return py_cal.remove_distortion(pos, cal_obj, flags)
    ...
    def targ_rec(self, img, par):
        return py_imgproc.targ_rec(img, par)
